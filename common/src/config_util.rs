...
lazy_static! {
     pub static ref APP_CONFIG :Config  = {
            ...
            _config.merge(config::File::with_name("conf/application")).unwrap();
            ...
            match _config.get_str("tl.app.mode")  {
                ...
                _ => {                      
                    _config.merge(config::File::with_name("conf/application_dev")).unwrap();             
                    false                   
                }                           
            };                              
            _config.merge(config::Environment::with_prefix("TL_APP")).unwrap();
            _config
     };
}
