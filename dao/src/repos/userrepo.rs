...
pub fn change_password(conn: &DbConnection, login_info_id: i64, old_password: &str,  new_password: &str ) -> Result<bool, String> 
{
        ...
        match target.load::<LoginInfoModel>(conn) {
            Ok(login_infos) if login_infos.len() == 1 =>  {
                    ...
                    if old_password_signed == login_info.password {
                        ...
                        match diesel::update(target).set(password.eq(signed_passwd)).execute(conn) {
                            Ok(_) => Ok(true),
